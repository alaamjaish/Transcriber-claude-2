...
import {
  ...
  sendAgentMessage,
  deleteChatSession,
} from '@/app/actions/agent-chat';
import { getTutorSettings, saveTeachingMethodology } from '@/app/actions/ai-tutor';
...
export function AIChatSidebar({ studentId, studentName, isOpen, onClose }: AIChatSidebarProps) {
  ...
  async function loadCurriculum() {
    ...
    if (settings) {
      setCurriculum(settings.teachingMethodology || '');
    }
  }
  ...
  async function handleSaveCurriculum() {
    ...
    try {
      const result = await saveTeachingMethodology(curriculum);
      if (result.success) {
  ...
  async function handleSendMessage() {
    ...
    try {
      // Use the NEW agent-powered chat!
      const result = await sendAgentMessage(currentSession.id, studentId, userMsg);
        ...
        <div className="w-64 bg-slate-900 dark:bg-slate-950 flex flex-col border-r border-slate-700">
          ...
          <div className="p-3 border-t border-slate-700 space-y-2">
            <button
              ...
              className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs font-medium text-white hover:bg-slate-750 transition-colors flex items-center justify-center gap-2"
            >
            ...
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
                  🤖 Powered by AI Agent - I can search lessons, analyze progress, and create content!
                </p>
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center p-6">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-3xl max-h-[80vh] flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Edit Curriculum & Teaching Methodology</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                This curriculum will be included in every AI conversation to guide responses
              </p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              <textarea
                ...
                placeholder="Describe your teaching methodology, curriculum structure, learning objectives..."
                className="w-full h-full min-h-[300px] rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-slate-700 resize-none font-mono"
              />
            </div>
            ...
            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 flex gap-3 justify-end">
              <button
                onClick={() => {
                  setShowCurriculumEditor(false);
                  loadCurriculum(); // Reset to original
                }}
              ...
              <button
                ...
                disabled={isSavingCurriculum}
                className="px-4 py-2 text-sm font-medium bg-slate-900 dark:bg-slate-700 text-white rounded-lg hover:bg-slate-800 dark:hover:bg-slate-600 disabled:opacity-50 transition-colors"
              >
                {isSavingCurriculum ? 'Saving...' : 'Save Curriculum'}
              </button>
            </div>
          </div>
        </div>
