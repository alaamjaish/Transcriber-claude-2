      ...
      <div className="w-full max-w-md space-y-6 rounded-xl border border-slate-800 bg-slate-950/95 p-6 shadow-2xl">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            {promptsLoading ? (
              <div className="text-sm text-slate-400 py-4">Loading your prompts...</div>
            ) : (
              <div className="space-y-2">
                {/* Default Prompt */}
                <label className="flex items-start gap-3 p-3 rounded-lg border border-slate-800 bg-slate-950/60 hover:bg-slate-900/60 cursor-pointer transition-colors">
                  <input
                    type="radio"
                    ...
                    className="mt-1 h-4 w-4 border-slate-600 bg-slate-950 text-sky-400 focus:ring-sky-500"
                    disabled={isPending}
                  />
                  <div className="flex-1">
                    <div className="text-sm font-medium text-slate-100">Default System Prompt</div>
                ...
                {/* Custom Prompts */}
                {prompts.map((prompt) => (
                  <label key={prompt.id} className="flex items-start gap-3 p-3 rounded-lg border border-slate-800 bg-slate-950/60 hover:bg-slate-900/60 cursor-pointer transition-colors">
                    <input
                      type="radio"
                      name="prompt-selection"
                      value={prompt.id}
                    ...
                    />
                    <div className="flex-1">
                      <div className="text-sm font-medium text-slate-100">{prompt.name}</div>
                ...
                {prompts.length === 0 && !promptsLoading && (
                  <div className="text-sm text-slate-500 py-4 text-center">
                    ...
                    <a href="/prompts" className="text-sky-400 hover:text-sky-300">Create your first prompt →</a>
                  </div>
                )}
              </div>
            )}
