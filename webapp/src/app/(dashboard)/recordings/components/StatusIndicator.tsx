...
export function StatusIndicator({ label, tone, durationMs }: StatusIndicatorProps) {
  const toneClass = {
    idle: "bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300",
    busy: "bg-amber-500/10 text-amber-600 dark:text-amber-300 border border-amber-400/40",
    live: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 border border-emerald-400/40",
    error: "bg-rose-500/10 text-rose-600 dark:text-rose-300 border border-rose-400/40",
  }[tone];
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 text-xs">
      ...
      <span className="rounded-full border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-900/60 px-3 py-1 text-slate-600 dark:text-slate-400">
        {minutes}:{seconds}
      </span>
    </div>
  );
