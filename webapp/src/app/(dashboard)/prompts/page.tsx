      ...
      >
        {errorMessage ? (
          ...
          <ul className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {prompts.map((prompt) => (
              <li key={prompt.id} className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/60 p-4 text-sm text-slate-700 dark:text-slate-300">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 line-clamp-1">{prompt.name}</h3>
                  <span className="rounded-full border border-slate-200 dark:border-slate-800 px-2 py-1 text-xs text-slate-600 dark:text-slate-400 shrink-0">
                    Custom
                  </span>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-xs">
                  <button
                    ...
                    className="rounded-md border border-slate-300 dark:border-slate-700 px-3 py-1 text-slate-900 dark:text-slate-100 hover:border-slate-400 dark:hover:border-slate-500"
                  >
                    Edit
                  </button>
                  <button
                    ...
                    className="rounded-md border border-rose-300 dark:border-rose-800 px-3 py-1 text-rose-600 dark:text-rose-300 hover:border-rose-400 dark:hover:border-rose-600"
                  >
                    Delete
                  </button>
                </div>
          ...
          <div className="p-8 text-center text-slate-600 dark:text-slate-400">Loading prompts...</div>
        ) : (
          <div className="rounded-lg border border-dashed border-slate-300 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/30 p-10 text-center text-slate-700 dark:text-slate-300">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">No custom prompts yet</h3>
